        ...
        while current_index < len(chat_links):
            ...
            try:
                ...
                try:
                    ...
                    log_message(username, f"Вступили в {link}")
                except errors.UserAlreadyParticipantError:
                    log_message(username, f"Уже в группе ({link})")
                except errors.FloodWaitError as e:
                    log_message(username, f"Флуд-контроль при вступлении: ждем {e.seconds} сек.")
                    await asyncio.sleep(e.seconds)
                    continue  # Повторим попытку с той же группой
                ...
                try:
                    ...
                    log_message(username, f"Ошибка получения entity для {link}: {str(e)}")
                    current_index += 1
                    continue
